...
def call_result_window_2(margin, c1_new, c2_new):
    ...
    lbl1 = Label(result_window, text='Bookmaker overround: ' + str(format(margin * 100, '.1f')) + '%')
    lbl2 = Label(result_window, text='Corrected coefficients:  ' + str(format(c1_new, '.2f')) + '  ' + str(format(c2_new, '.2f')))
    lbl3 = Label(result_window, text='New overround: ' + str(format(abs((1 / c1_new + 1 / c2_new) - 1) * 100, '.1f')) + '%')
...
def call_result_window_3(margin, c1_new, c2_new, c3_new):
    ...
    result_window.iconbitmap(icon)
    lbl1 = Label(result_window, text='Bookmaker overround: ' + str(format(margin * 100, '.1f')) + '%')
    lbl2 = Label(result_window, text='Corrected coefficients:  ' + str(format(c1_new, '.2f')) + '  ' + str(format(c2_new, '.2f')) + '  ' + str(format(c3_new, '.2f')))
    lbl3 = Label(result_window, text='New overround: ' + str(format(abs((1 / c1_new + 1 / c2_new + 1 / c3_new) - 1) * 100, '.1f')) + '%')
    btn = Button(result_window, text='OK', width=20, command=lambda: result_window.destroy())
    lbl1.pack(pady=10)
    lbl2.pack(pady=10)
    lbl3.pack(pady=10)
...
def two_way_button_action():
    ...
    two_way_window.title('Coefficients input')
    two_way_window.geometry('300x150')
    two_way_window.iconbitmap(icon)
    lbl = Label(two_way_window, text='Enter the coefficients')
    frame = Frame(two_way_window)
    entry1 = Entry(frame, width=10, justify='center')
    entry2 = Entry(frame, width=10, justify='center')
    btn = Button(two_way_window, text='Recalculate the coefficients', command=lambda: two_way_zeroing(float(entry1.get()), float(entry2.get())))
...
def three_way_button_action():
    ...
    three_way_window.title('Coefficients input')
    three_way_window.geometry('300x150')
    three_way_window.iconbitmap(icon)
    lbl = Label(three_way_window, text='Enter the coefficients')
    frame = Frame(three_way_window)
    entry1 = Entry(frame, width=10, justify='center')
    entry2 = Entry(frame, width=10, justify='center')
    entry3 = Entry(frame, width=10, justify='center')
    btn = Button(three_way_window, text='Recalculate the coefficients', command=lambda: three_way_zeroing(float(entry1.get()), float(entry2.get()), float(entry3.get())))
    lbl.pack(side=TOP, pady=15)
    frame.pack()
    entry1.pack(side=LEFT, padx=20, pady=10)
...
def main():
    root = Tk()  
    root.title('Overround zeroing')
    root.geometry('300x100')
    root.iconbitmap(icon)
    lbl = Label(root, text='Choose event type')  
    btn1 = Button(root, text='2-way (12)', command=two_way_button_action)
    btn2 = Button(root, text='3-way (1X2)', command=three_way_button_action) 
